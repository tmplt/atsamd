...
pub struct ClockInputModeWithCFD {
    dfll: Dfll,
}
impl Mode for ClockInputModeWithCFD {}
impl Sealed for ClockInputModeWithCFD {}

pub struct XOscInputModeWithCFD<X: XOscNum> {
    xout: XOut<X>,
    crystal_current: CrystalCurrent,
    dfll: Dfll,
}
impl<X: XOscNum> Mode for XOscInputModeWithCFD<X> {}
impl<X: XOscNum> Sealed for XOscInputModeWithCFD<X> {}
...
impl<X: XOscNum> XOscConfig<X, ClockInputModeWithCFD> {
    /// TODO
    #[inline]
    pub fn from_clock(
        mut token: XOscToken<X>,
        ...
        let xin = xin.into().into_floating_disabled();
        token.set_clock_failure_detection(true);
        token.from_clock();
        // TODO
        Self {
            token,
            mode: ClockInputModeWithCFD { dfll },
            xin,
            freq: freq.into(),
        }
    }
    ...
    /// TODO
    #[inline]
    pub fn set_clock_failure_detection(mut self, cfden: bool) -> Self {
        self.token.set_clock_failure_detection(cfden);
        self
    }

    /// Controls the prescaler for the safe clock used when
    /// clock failure detection. `DFFL48` is divided by the prescaler
    #[inline]
    pub fn set_clock_failure_detection_prescaler(mut self, prescale: CFDPRESC_A) -> Self {
        self.token.set_clock_failure_detection_prescaler(prescale);
        self
    }

    /// TODO
    #[inline]
    pub fn free(self) -> (XOscToken<X>, XIn<X>, Dfll) {
        (self.token, self.xin, self.mode.dfll)
    }
}
