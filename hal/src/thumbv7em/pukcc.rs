...
impl Pukcc {
    ...
    pub fn enable(mclk: &mut MCLK) -> Result<Self, SelfTestFailure> {
        // TODO: Alleged RNG initialization. Passthrough constructor?
        unsafe {
    ...
    pub fn zp_ecdsa_sign<C: Curve>(
        ...
        // 32-byte zero padding on a MSB side of every parameter is required by PUKCC
        // algorithms. Endianess of parameters themselves is resolved within macro.
        // Little endianess requires padding *after* a parameter as MSB is placed on
        // high addresses.
        ...
        // Base point Z coordnate should be zero.
    ...
    ///
    /// Input parameters:
    /// - `signature`: `&mut [u8]` of length `2 * `[`Curve::SCALAR_LENGTH`]
    ...
    pub fn zp_ecdsa_verify_signature<'a, C: Curve>(
        &self,
        signature: &'a [u8],
        hash: &'a [u8],
        public_key: &'a [u8],
        ...
        copy_to_cryptoram! {
            ...
            // Signature layout:
            //   [ R: 32-bytes little endian ][ 0_u32 ]..
            // ..[ S: 32-bytes little endian ][ 0_u32 ]
            ...
            // Public key layout:
            //   [ X coordinate: 32-bytes little endian ][ 0_u32 ]..
            // ..[ Y coordinate: 32-bytes little endian ][ 0_u32 ]
            // ..[ Z coordinate: 32-bytes little endian ][ 0_u32 ]
