//! Module that defines low-level constructs required for an interaction with
//! PUKCC
#![allow(missing_docs)]
...
pub struct ZpEcDsaQuickVerify {
    pub pu1ModCnsBase: pu1, // ModBase and CnsBase at (ModBase + u2ModLength + 4)
    pub pu1PointABase: pu1,
    pub pu1PointPublicKeyGen: pu1,
    pub pu1PointSignature: pu1,
    pub pu1OrderPointBase: pu1,
    pub pu1AWorkBase: pu1, // ABase and Workspace at (ABase + u2ModLength + 4)
    pub pu1HashBase: pu1,
    pub u2ModLength: u2,
    pub u2ScalarLength: u2,
}
...
#[derive(Copy, Clone, Default, Debug)]
pub struct PrimeGen {
    pub nu1RndBase: nu1, // (3*u2NLength + 6) words
    pub nu1NBase: nu1,   //
    pub nu1CnsBase: nu1,
    pub nu1PrecompBase: nu1, // (u2NLength + 2) words
    pub nu1RBase: nu1,       // (Significant length of 'N' without the padding word)
    pub nu1ExpBase: nu1,     // (u2NLength) words
    pub u2NLength: u2,
    pub __Padding0: nu1,
...
#[derive(Copy, Clone, Default, Debug)]
pub struct Div {
    pub nu1ModBase: nu1,   // Denominator
    pub nu1WorkSpace: nu1, // Workspace
    pub u2ModLength: u2,   // Denominator length

    pub nu1RBase: nu1, // Result copy
    pub __Padding0: u2,
    pub nu1QuoBase: nu1, // Quotient
    pub nu1NumBase: nu1, // Numerator, Remainder
    pub __Padding1: u2,
    pub __Padding2: u2,
    pub u2NumLength: u2, // Numerator length
}
...
pub struct CRT {
    pub nu1XBase: nu1,       // x
    pub nu1ModBase: nu1,     // Primes P and Q
    pub nu1PrecompBase: nu1, // R, precomp
    pub __Padding0: nu1,
    pub pfu1ExpBase: pfu1, // Exponent
    pub u2ModLength: u2,   // N=length of P or Q in bytes
    pub u2ExpLength: u2,   // Exponent length in bytes
    pub u1Blinding: u1,    // Exponent blinding using a 32-bits Xor
    pub __Padding1: u1,
    pub __Padding2: u2,
}
...
#[derive(Copy, Clone, Default, Debug)]
pub struct Rng {
    pub nu1XKeyBase: nu1, /* Pointer to the Input and Output XKEY value of length u2XKeyLength
                           * bytes */
    pub nu1WorkSpace: nu1, // Pointer to the workspace of length 64 bytes
    pub u2XKeyLength: u2,  // Length in bytes multiple of 4 of XKEY, XSEED[0], XSEED[1]

    pub nu1XSeedBase: nu1, /* Pointer to the Input value of XSEED[0] and XSEED[1] of length
                            * (2*u1XKeyLength + 4) */
    pub nu1WorkSpace2: nu1, // Pointer to the WorkSpace2 Of SHA (HICM)
    pub nu1QBase: nu1,      // Pointer to the Input prime number Q of length 160 bits = 20 bytes
    pub nu1RBase: nu1,      // (Significant length of 'N' without the padding word)
    pub u2RLength: u2,      // length of the resulting RNG
...
pub struct GCD {
    ...
    pub nu1ABase: nu1, // a
    pub nu1ZBase: nu1, // z
    pub u2Length: u2,  // x,y,a,z length
}
...
#[derive(Copy, Clone, Default, Debug)]
pub struct ExpMod {
    pub nu1XBase: nu1, // (3*u2NLength + 6) words LSW is always zero
    pub nu1ModBase: nu1,
    pub nu1CnsBase: nu1,
    pub nu1PrecompBase: nu1, // xxx words LSW is always zero
    pub pfu1ExpBase: pfu1,   // u2ExpLength words
    pub u2ModLength: u2,
    pub u2ExpLength: u2,
    pub u1Blinding: u1, // Exponent blinding using a 32-bits Xor
    pub __Padding0: u1,
